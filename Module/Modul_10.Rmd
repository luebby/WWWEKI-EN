...
- use the graph to draw the right consequences for causal inference;
...
*Press `Next` once you are done with the graph*
...
3. the *salary* a person gets depends on the *ability*, on *board* membership and on  *gender*. Therefore, one arrow each from *ability*, *board* and *gender* to *salary*.
...
Men earn on average `r round(mean(salary ~ gender, data = GPG))[1]` more than women, whose average salary here is `r round(mean(salary ~ gender, data = GPG))[2]`.
...
The following vidoeo shows a short introduction to the operation of DAGitty using this example:
