...
As a reminder, causality flows along with the arrows. $$A \rightarrow B$$ states that $B$ *listens* to $A$. For example, if it rains, ($A$), the road gets wet ($B$). 

In a chain, we simply add a third variable: $$A \rightarrow B \rightarrow C.$$ 
For example: if it rains, ($A$) the road gets wet ($B$) and thus it gets slippery ($C$).
...
Suppose <green>learning</green> ($\color{green}{X}$) leads to <violet>knowledge</violet> ($\color{violet}{Z}$), that is, by learning you acquire knowledge. Furthermore, <violet>knowledge</violet> ($\color{violet}{Z}$) leads to <blue>understanding</blue> ($\color{blue}{Y}$), i.e., through your knowledge you come to an understanding.
...
question("In the language of causal graphs: Is understanding ($Y$) a child of learning ($X$)?",
         answer("Yes"),
         answer("No", correct = TRUE, message = "Understanding is a child of knowledge ($Z$). Knowledge is in turn a child of learning. Thus, understanding is not a child of learning, but it is a descendant. Understanding *listens* directly only to knowledge, i.e., the value of understanding depends directly only on knowledge."),
         allow_retry = TRUE,
         correct = "Great, Correct!",
         incorrect = "Unfortunately, this was wrong. Maybe take another look at Module 2.")
```

## Mediator

In such scenarios, $$\color{green}{X} \rightarrow \color{violet}{Z} \rightarrow \color{blue}{Y}$$, the variable in the middle &ndash; here $\color{violet}{Z}$ &ndash; is called a **mediator**.

```{r mediator, echo=FALSE}
question("Suppose a whether a candidate is promoted depends on their gender due to discrimination, and the future salary of course depends on the promotion. Which variable is a mediator here?",
         answer("Gender"),
         ...
         answer("salary", message = "The causal model described is $\text{gender} \rightarrow \text{support} \\rightarrow \\text{salary}$."),
         allow_retry = TRUE,
...
To study the causal effect of the cause ($\color{green}{X}$) on the effect ($\color{blue}{Y}$), we do not need to know the value of the mediator ($\color{violet}{Z}$). So, for example, if we want to know how <green>gender</green> affects <blue>content</blue> overall, we do not need information about <violet>promotions</violet>.
...
And if we were interested in the extent to which the effects of gender are mediated by promotion, the analysis target would be the indirect effect.
...
Recall (see also Module 2) that $U$ denotes unknown causes and $f$ the functions by which values are assigned to the variables.
