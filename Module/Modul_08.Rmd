...
As you can see: Children of women who <green>breastfed</green> exclusively for at least four months were less likely to be <blue>overweight</blue> at age six than children who weren't breastfed &ndash; in this fictional study.


This is level of **description** (see Module 3).<br>
...
- <green>Breastfeeding</green> (<purple>Yes</purple>, <orange>No</orange>) could also potentially depend on whether or not the mother is a college <violet>graduate</violet> ist.
...
Suppose that we are interested in a population where 30% of women are graduates.
...
Suppose that for our <u>data collection</u>, there is a relationship between the mother's level of education (<violet>graduate</violet>) and her participation in the study.
...
pgsti <- ggplot(d, aes(x = x, y = y, color = Sample)) +
  geom_tile(color = "white", size = .5, aes(fill = Sample), alpha = .2) +
  theme_void() +
  geom_text(family='fontawesome-webfont', size = 8, aes(label = graduate), colour = breastfeeding) +
...
In this random sample, the probability of <purple>breastfeeding</purple> with $`r round(pdz,2)`$ no longer deviates systematically from the *true* probability in the target population. 
...
question("Can the causal effect of breastfeeding on overweight be estimated by a direct comparison of children whose mothers breastfeed vs. do not breastfeed?",
         answer("Yes"),
         answer("No", correct = TRUE, message = message),
		allow_retry = TRUE,
		correct = random_praise(),
...
Knowing how many <purple>breastfed</purple> children did become overweight is nice, but we still lack at least a comparison to  children who were <orange>not breastfed</orange>.

In the fictitious study, as reported in the headline, the probability of overweight is $\color{purple}{`r poverweightbreastfeeding`}$ for <purple>breastfed</purple> children, and it is $\color{orange}{`r poverweightkbreastfeeding`}$ for children who were <orange>not breastfed</orange>.
...
A comparison of the proportion <blue>overweight</blue> according to <green>breastfeeding</green> reveals:
...
question("Does the observational study data suggest that the average causal effect of breastfeeding on the probability of overweight is $0.36$?",
         answer("Yes"),
         answer("No", correct = TRUE, message = message),
		allow_retry = TRUE,
		correct = random_praise(),
...
pbeob <- ggplot(d, aes(x = x, y = y, color = Overweight)) +
  geom_tile(color = "white", size = .5, aes(fill = Overweight), alpha = .2) +
  theme_void() +
...
But: Graduates <purple>breastfeed</purple> more often than non-graduates &ndash; and children of graduates are less likely to become overweight.



## Randomized experiment

Assuming the causal model with only one confounder (here: graduate), we could simply adjust for this variable, as we have already learned.
But what if we did not collect the variable in our data?
...
A comparison of the probabilities for <blue>overweight</blue> according to <green>breastfeeding</green> now yields the following estimate of the average causal effect:
...
We need additional knowledge about how the data came to be &ndash; what additional variables influenced the variables of interest, or whether, for example, there was a random intervention.


***

*Note*: For reasons of statistical precision, it is appropriate to include the variable <violet>graduate</violet> in the analysis even in a randomized experiment.
...
In this module, you have learned statistical methods for optimally designing data collection, if possible, so that no differences are found in (4). 
Ideally, we have a random sample; ideally, there is random assignment to experimental groups.
