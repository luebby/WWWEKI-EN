...
At the beginning of the millennium, Robert Matthews took the trouble to collect data to answer the question ([Quelle](https://doi.org/10.1111/1467-9639.00013)):
...
As you can see, there are countries with many storks &ndash; where many babies are born. And countries with comparatively few storks &ndash; and at the same time few births.
...
question("The correlation coefficient between the number of storks and the number of births is $r_{\\color{green}{x},\\color{blue}{y}} = +0.62$. From this, can you conclude the correlation coefficient between the number of births and the number of storks?",
         answer("$r_{\\color{blue}{y}, \\color{green}{x}} = -0.62$."),
         answer("$r_{\\color{blue}{y}, \\color{green}{x}} = 1/0.62 = 0.62^{-1}$."),
         answer("$r_{\\color{blue}{y}, \\color{green}{x}} = +0.62$.", correct = TRUE, message = paste(message, "So maybe the babies are delivering the storks?")),
         allow_retry = TRUE,
         correct = random_praise(),
         incorrect = random_encouragement()
         )
...
(Of course, there are potentially numerous other common causes of the <green>number of storks</green> ($\color{green}{X}$) and the <blue>number of births</blue> ($\color{blue}{Y}$).)
...
question("Does the value of area ($\\color{violet}{Z}$) causally depend on the number of storks ($\\color{green}{X}$)?",
         answer("Yes"),
         answer("No", correct = TRUE, message = "The causal model described is $\\text{number of storks} \\leftarrow \\text{area}$. The number of storks *listens* to the area, but the area **does not** listen to the number of storks. More storks can't change the area, but the area can change the number of storks."),
         allow_retry = TRUE,
         correct = random_praise(),
         incorrect = random_encouragement()
         )
...
Both the value of $\color{green}{X}$ and the value of $\color{blue}{Y}$ depend causally on $\color{violet}{Z}$, the structural causal model looks as follows:
\begin{eqnarray*}
...
question("In the chain, does the value of $\\color{blue}{Y}$ change when there is an intervention on $\\color{green}{X}$ ($do(x)$)? ",
         answer("Yes"),
...
If $\color{green}{X}$ is changed ($do(\color{green}{X}=\color{green}{x})$), the distribution of $\color{violet}{Z}$ does not change, neither does the distribution of $\color{blue}{Y}$.
...
And, of course, there could be additional confounders.


## Summary

:::{.box}
To determine the causal effect of $X$ on $Y$ in a fork $$X \leftarrow Z \rightarrow Y$$, the confounder $Z$ must be taken into account 
If $Z$ is simply ignored, the fork remains open and a non-causal associations between $X$ and $Y$ distorts the analysis results.
...
*Note*: We did not set up the random number generator here, so there will be random variation in the outcome each time you press `Run Code`.
