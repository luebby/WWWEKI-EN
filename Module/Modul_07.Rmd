...
message <- "The estimated linear model of the sample refers to the observed data ($X=x$), i.e., the slope describes the observed change in the mean, not the effect of an action $do(X=x)$ (cf. Module 2)."
...
```{r graph2, echo=FALSE}
message <- "Apart from the direct effect via $X \\rightarrow Y$, there is also the non-causal $X \\leftarrow Z \\rightarrow Y$. Here, the **area** is a **confounder**."
...
Instead of an estimated slope of $\hat{\beta}_1=`r round(coef(erglm1))[2]`$ in the model *without* accounting for <violet>living area</violet>, we find  $\hat{\beta}_1=`r round(coef(erglm2))[2]`$ *with* accounting it.
...
This course was supported by a grant from the German Federal Ministry of Education and Research, grant number 16DHBQP040.
 Förderkennzeichen 16DHBQP040 gefördert.
